...
pool_auMessage724 = redis.ConnectionPool(host='localhost', port=6379, db=3)
r_auMessage724 = redis.StrictRedis(connection_pool=pool_auMessage724)

pool_A_DxtInformation = redis.ConnectionPool(host='localhost', port=6379, db=4)
r_A_DxtInformation = redis.StrictRedis(connection_pool=pool_A_DxtInformation)

pool_B_DxtMarketHouse = redis.ConnectionPool(host='localhost', port=6379, db=5)
